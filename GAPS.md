# Gaps — gap-analysis-standard

> **Standard:** Rural Peds Gap Analysis Standard v1.0
> **Spec:** https://github.com/ruralpeds/gap-analysis-standard/blob/main/SPEC.md
> **Last updated:** 2026-04-28

---

## Active

| ID | Title | Priority | Category | Created | Notes |
|----|-------|----------|----------|---------|-------|
| GAP-001 | Create ruralpeds/gap-dashboard aggregator repo | P1 | infrastructure | 2026-04-28 | see docs/aggregation.md |
| GAP-002 | Tag v1.0 and v1 floating release | P1 | infrastructure | 2026-04-28 | enables @v1 pinning in consumers |
| GAP-004 | Bootstrap ruralpeds/textbook | P2 | infrastructure | 2026-04-28 | has 18 gaps already partially written |
| GAP-005 | Bootstrap timothyhartzog/Patient-simulation-julia | P2 | infrastructure | 2026-04-28 | convert SIMULATION_GAP_ANALYSIS.json |
| GAP-006 | Bootstrap ruralpeds/rust-sci-core | P2 | infrastructure | 2026-04-28 | |
| GAP-007 | Add aggregate_gaps.py to gap-dashboard design | P2 | infrastructure | 2026-04-28 | see docs/aggregation.md |
| GAP-008 | Write JSON-to-GAPS.md conversion script | P2 | infrastructure | 2026-04-28 | for repos with existing JSON gap files |
| GAP-009 | Add GitHub Actions CI for test_update_gaps.py | P2 | infrastructure | 2026-04-28 | run pytest on every PR |
| GAP-010 | Install GapsCLI.jl as shell alias in Mac Studio | P3 | infrastructure | 2026-04-28 | |

---

## Completed

| ID | Title | Priority | Category | Completed | PR |
|----|-------|----------|----------|-----------|-----|

---

| GAP-003 | Bootstrap ruralpeds/Peds (pilot repo) | P1 | infrastructure | 2026-04-28 | #pilot |
## Abandoned

| ID | Title | Reason |
|----|-------|--------|
